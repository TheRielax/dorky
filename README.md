...
- **🕵️‍♂️ Target Recon & OSINT Suite (Ashok Inspired)**:
  ...
  - **🔬 Web Technology, CMS & Site Tree Fingerprinter**: Lightweight HTTP header, DOM, and structural site tree analyzer detecting CMS platforms (WordPress, Joomla, Drupal), Frontend frameworks (React, Next.js, Vue), Backends (PHP, Laravel, Django), and WAFs (Cloudflare). Automatically parses `/robots.txt` and `/sitemap.xml` (including recursive child sitemap indexes) to map top-level directory trees, reveal `Disallow` paths, perform structural path signature detection, and export complete structural dumps.
  ...
  - **📧 Email Address OSINT & Breach Footprinting**: 100% offline parsing and MX mail exchange resolution (`dnspython`) categorizing mail infrastructures (Free Webmail, Encrypted Privacy Mail, Google Workspace, Microsoft 365, Self-Hosted). Automatically generates strictly grouped Boolean footprint dorks for data breach spreadsheet leaks (`xls/csv/sql`), Pastebin dumps, and professional profile mentions.
  - **👤 Username & Social Cross-Platform Footprinting**: Live HTTP status probing across 13 major developer, social, and gaming platforms (GitHub, Reddit, Telegram, Steam, HackTheBox, Pastebin, etc.) confirming active profiles in real-time, accompanied by targeted cross-platform boolean dorks.
  - **🌐 Passive DNS, WHOIS & Security Posture (SPF/DMARC Auditor)**: Complete passive infrastructure recon featuring a multi-layer WHOIS contact harvester (extracting and cleanly formatting Registrant, Administrative Contact, and Technical Contact street addresses and organizations alongside the unabridged raw WHOIS record output). Resolves authoritative IPv4 records, evaluates email spoofing and anti-phishing vulnerability posture (`v=spf1` / `_dmarc`), and generates inclusive infrastructure dorks.
